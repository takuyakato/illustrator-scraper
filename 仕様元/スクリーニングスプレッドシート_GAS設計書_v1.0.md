# スクリーニング用Googleスプレッドシート & GAS 設計書
...
イラストレーター候補者のスクリーニング作業を、Notion を開くことなく Google スプレッドシート内で完結させる。スクリーナーは判定作業のみを行い、「○」判定されたレコードは GAS で自動的に Notion マスターDB へ転記される。
...
| シート名 | 用途 | 主担当 |
|---|---|---|
| **候補プール** | メインの作業シート | スクリーナー |
| 転記ログ | Notion転記の実行履歴 | GAS自動記録 |
| 設定 | GAS の動作設定値 | オーナー |
| README | スクリーナー向け簡易マニュアル | − |
...
| **J** | **判定** | **SELECT** | **スクリーナー** | **`○` / `保留` / `×` / （空）** |
| **K** | **画力ランク候補** | **SELECT** | **スクリーナー** | **`S` / `A` / `B` / `C` / （空）** |
| **L** | **コメント** | **TEXT** | **スクリーナー** | **自由記述** |
...
- 1行目（ヘッダー）: 固定
- A〜I列・M〜O列: 「スクリーナー編集不可」で保護（GAS・Claude Code のみ書き込み）
- J〜L列: スクリーナーが自由に編集可
...
| A5 | 見つけた人（デフォルト） | `（スクリーナーメール→人名マッピング）` | GAS内で別途変換 |
...
function createNotionPage(target, config) {
  ...
  const properties = {
    ...
    ...(target.screenerComment ? {
      '備考': {
        rich_text: [{
          text: { content: `[スクリーナー] ${target.screenerComment}` }
        }]
      }
    } : {})
  };
