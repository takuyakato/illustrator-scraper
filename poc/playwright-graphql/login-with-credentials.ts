/**
 * 手段1 Playwright+GraphQL: ID/PASS をローカル env から読むログイン補助。
 *
 * 注意:
 * - 認証情報は poc/.env.local にだけ置く（gitignore 済み）
 * - 2FA / CAPTCHA / 追加の username 確認が出た場合はブラウザ上で手動対応する
 */
...
const USER_DATA_DIR = resolve(__dirname, '.chrome-profile');
...
if (!loginId || !password) {
  console.error('X_LOGIN_ID / X_LOGIN_PASSWORD が未設定です。poc/.env.local に設定してください。');
  process.exit(1);
}
