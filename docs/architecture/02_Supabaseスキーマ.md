...
ただし Notion の `multi_select` は運用中に値が増えやすいため、`owner_confirmed_by` / `style_tags` は `TEXT[]` で保持します。
...
#### Legacy カラム・Notion非同期（7カラム）

migration 18 後も Supabase に保持するが、Notionには同期しない旧プロパティ群。削除済み legacy カラムは `migration_snapshot` または統合先カラムから必要時に参照・復元する。
...
CREATE TABLE illustrators (
  ...
  rank                            rank_enum,
  owner_confirmed_by              TEXT[] NOT NULL DEFAULT '{}'::TEXT[],
...
| `failure_key` | `TEXT` | YES | ー | 未解決行で一意 | 同一失敗を畳み込むためのキー |
| `occurrence_count` | `INTEGER` | YES | `1` | `CHECK (occurrence_count >= 1)` | 同一未解決失敗の発生回数 |
| `created_at` | `TIMESTAMPTZ` | YES | `NOW()` | ー | 失敗発生日時 |
| `last_seen_at` | `TIMESTAMPTZ` | YES | `NOW()` | ー | 同一失敗の最終発生日時 |
...
COMMENT ON COLUMN sync_failures.failure_key IS '未解決失敗の重複判定キー。同一キーの未解決失敗は1行に畳む。';
COMMENT ON COLUMN sync_failures.occurrence_count IS '同一 failure_key の未解決期間中の発生回数。';
COMMENT ON COLUMN sync_failures.last_seen_at IS '同一 failure_key の最終発生時刻。通知や監視では created_at よりこちらを優先する。';
```
...
| `idx_sync_failures_unresolved_failure_key` | sync_failures | failure_key | 部分 UNIQUE | 未解決の同一失敗を1行に畳む |
| `idx_sync_failures_unresolved_last_seen` | sync_failures | last_seen_at DESC | 部分 | 最近再発した未解決失敗の通知・監視 |
...
CREATE UNIQUE INDEX idx_sync_failures_unresolved_failure_key
  ON sync_failures (failure_key) WHERE resolved_at IS NULL;
CREATE INDEX idx_sync_failures_unresolved_last_seen
  ON sync_failures (last_seen_at DESC) WHERE resolved_at IS NULL;
...
`update_updated_at()` は、`last_synced_*_at` と `updated_at` だけが変わった UPDATE では `updated_at` を据え置く。これにより、同期完了タイムスタンプの書き込みが次サイクルの同期対象を作る無限ループを防ぐ。

実データ変更の判定は `to_jsonb(NEW)` / `to_jsonb(OLD)` から同期メタデータ系キーを除外して比較する。実データカラム名を関数内に手書き列挙しないため、`illustrators` の通常カラム追加・削除にトリガー関数が自動追従する。
...
### 7.6 自動遷移関数：連絡中 → 返信なし（14日経過）

```sql
-- 「連絡中」かつ「連絡した日から14日経過」のレコードを「返信なし」に自動遷移
...
- `updated_at` は実データ変更時に自動更新される
- `last_synced_to_notion_at` / `last_synced_from_notion_at` / `last_synced_to_sheet_at` / `last_synced_from_sheet_at` / `updated_at` だけの更新では、トリガーが `updated_at` を据え置く
...
- ループ防止の基本：同期完了タイムスタンプだけの書き込みでは `updated_at` を進めず、実データ変更だけを `updated_at > last_synced_to_*_at` の対象にする
