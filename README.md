...
ログイン成功後、`.scraper/x-storage-state.json` が生成されます。このファイルはXのログイン状態を含むため git に入れません。

### GitHub Actions 定期実行

`Scraper Seed Batch` workflow は GitHub Secret `X_STORAGE_STATE_JSON` から `.scraper/x-storage-state.json` を復元して実行します。

登録手順:

```bash
gh secret set X_STORAGE_STATE_JSON < .scraper/x-storage-state.json
```

X側でログイン状態が期限切れになった場合は、ローカルで `npm run scraper:login` を再実行し、同じ `gh secret set` で Secret を更新してください。

実行結果の概要は workflow artifact `scraper-run-summary` に `tmp/scraper-run-all.json` として保存されます。
